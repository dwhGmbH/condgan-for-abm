...
class CondGANTrainer():
    ...
    def __init__(self, config:CondGANConfig,use_cuda=False):
        ...
        """
        self.config = config
        assert config.get_critic_hyperparams()['sequence'][0] == config.get_generator_hyperparams()['sequence'][0], 'input layers of critic and generator must match'
    ...
    def train(self, parameters:np.array,values:np.array):
        ...
        """
        assert parameters.shape[0] == len(values), f'dimension mismatch between training parameters ({parameters.shape[0]}) and values ({len(values)})'
        ...
        assert self.inputDimension == self.config.get_critic_hyperparams()['sequence'][0]-1, f'dimension mismatch between training parameters ({parameters.shape[0]}) and input layer ({self.config.get_critic_hyperparams()['sequence'][0]}), whereas the latter should be by one larger than the prior.'
        ...
        self.metrics = [self._lossGeneratorName, self._lossCriticName] #specify tracked metrics. Always track loss values
    ...
    def loss_critic_base(self, values_data:torch.Tensor, values_latent:torch.Tensor, output_data:torch.Tensor, output_latent:torch.Tensor) -> torch.Tensor:
        """
    ...
    def loss_critic_log(self, values_data:torch.Tensor, values_latent:torch.Tensor, output_data:torch.Tensor, output_latent:torch.Tensor) -> torch.Tensor:
        """
    ...
    def loss_critic_wasserstein_wc(self,values_data:torch.Tensor, values_latent:torch.Tensor, output_data:torch.Tensor, output_latent:torch.Tensor) -> torch.Tensor:
        """
    ...
    def loss_critic_wasserstein_gp(self,values_data:torch.Tensor, values_latent:torch.Tensor, output_data:torch.Tensor, output_latent:torch.Tensor) -> torch.Tensor:
        """
        ...
        if self.use_cuda:
            ...
            alpha = alpha.cuda()
        values_interpolated = alpha * values_data + (1 - alpha) * values_latent #compute some random but representative point on the value-space
        ...
        gradients = torch.autograd.grad(outputs=output_interpolated, inputs=values_interpolated,
                               ...
                               create_graph=True, retain_graph=True, only_inputs=True)[0] # compute the gradients w.r. to the input
